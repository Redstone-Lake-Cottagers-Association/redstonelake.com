...
| Air quality | Observed: OpenAQ v3 (`OPENAQ_API_KEY` Fly secret) via bbox reaching the MECP **Dorset** monitor 31 km NW (OpenAQ's radius param caps at 25 km — must use bbox; one stale duplicate Dorset location exists, stuck in 2023, filtered by a 6-hour freshness guard). Fallback + hourly forecast: `https://air-quality-api.open-meteo.com/v1/air-quality` (no key; CAMS model) | `/api/air-quality` → weather modal “Air Quality” tab (24-h forecast, 30-day history, 7-day outlook) | US AQI computed from PM2.5 (EPA 2024 breakpoints); source disclosed in tooltip/panel |
